...
Usage:
    python cluster.py --output clustered_data.csv
    python cluster.py --e-dist 15 --e-days 10 --limit 50000
"""
...
def main():
    ...
    try:
        ...
        if args.limit:
            query += f"\nLIMIT {args.limit}"
            logger.info("Loading %d specimens from database...", args.limit)
        else:
        ...
        # Drop rows without coordinates or dates (should be minimal due to WHERE clause)
        ...
        logger.info("Processing %d specimens...", len(df))
