...
## Data
- `data/clustered_expeditions_redacted.csv` is a redacted example output tracked with Git LFS.
- It was generated by running `expedition-cluster --redact --output data/clustered_expeditions_redacted.csv` against the CAS botany database.
- Pull LFS data with `git lfs pull` after cloning.
- The full CAS botany dump is not distributed with this repository.
